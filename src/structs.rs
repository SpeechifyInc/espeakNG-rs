...
use crate::{bindings, utils, utils::StringFromCPtr};
...
pub enum Parameter {
    ...
    Rate = 1,
    /// Volume of speech. Values should be 0-100 as greater values may produce amplitude
    /// compression or distortion.
    Volume = 2,
    /// Base pitch, default 50. Values must be between 0-100 inclusive.
    Pitch = 3,
    /// The pitch range, where 0 is monotone and 50 is normal. Values must be between 0-100
    /// inclusive.
    Range = 4,
    ...
    /// - 1 = sound icon
    /// - 2 = spelling
    /// - 3 or higher, by raising pitch. The value is the amount of Hz by which the pitch of each
    ///   capitalised word is raised.
    Capitals = 6,
